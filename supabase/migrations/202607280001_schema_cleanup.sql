drop index if exists public.responses_telegram_user_id_idx;
drop index if exists public.responses_study_route_idx;

drop trigger if exists responses_set_updated_at on public.responses;
drop function if exists public.set_updated_at();
alter table public.responses drop column if exists updated_at;
...
begin
  ...
  if jsonb_typeof(p_statistics) <> 'object'
    or (select count(*) from jsonb_object_keys(p_statistics)) <> 8
